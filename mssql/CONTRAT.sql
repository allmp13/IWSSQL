SELECT
    left(OS.C_REF, 2) AS CODE_METIER,
    OS.C_REF,
    P.L_PROCED,
    CASE
...
FROM suiteisilog.LIGNECTR LC
    ...
    LEFT JOIN suiteisilog.PROCED P ON P.C_PROCED=OS.C_PROCED
WHERE
    CTR.C_TYPECTR in('SLA','UC', 'OLA')
    AND LC.D_ARCHIVE IS NULL --AND OS.C_REF like 'TARI%'
    AND OS.C_STOBJ = 'EE'  -- Service en exploitation EE
ORDER BY
    OS.C_REF,
    CTR.C_TYPECTR,
    LC.L_NIVEAU
